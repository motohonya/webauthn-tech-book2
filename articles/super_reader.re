...
@<br>{}このフローの中でも書かれていますがRelying Partyに認可コードを渡すために「ユーザー認証」を行っています。
...
@<br>{}OIDCのAuthorization CodeフローにFIDO(WebAuthn)のフローを追加したものが@<img>{fido_in_oidc}です。
...
まずはOP側の認証を行う必要があります。@<img>{yahoo_fido_2}はOPであるYahoo! JAPANのログイン画面です。未ログイン状態からログインするためにIDを入力して、「次へ」ボタンを押します。
...
指紋認証に成功したら、サーバー側に対してassertionが送られ、署名されたデータをOPのFIDOサーバーに保存してある公開鍵で検証を行います。
...
以上の具体例からもOIDCとFIDOは共存することが可能であることがわかります。
...
OIDCの仕様の中にも認証をどうするかについて書かれた@<b>{Self-issued}という仕様があります。
