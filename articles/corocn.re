...
@corocnといいます。
この本を執筆している現在は、株式会社キャスターという会社でのエンジニアリングが本業で、
プライベートでAuth0というIDaaSのアンバサダーをしています。
普段はRuby on Rails （RoR）+ Vue.jsで小さめのWebサービスを書いていることが多いです。
ガッツリ仕事で認証関連の仕事をしているわけではなく、セキュリティエンジニアでもありませんのであしからず。
認証技術は好きですが認可はそうでもないです。

FIDO2、WebAuthnが何者なのかという説明は、書きたいことの本質ではないので省きます。（多分他の誰かが書いてるだろうし。）
もし復習しておきたいという方は、@agektmr 氏の
「パスワードの不要な世界はいかにして実現されるのか - FIDO2 と WebAuthn の基本を知る」@<fn>{agektmr_fido_document}が非常によくまとまっているのでオススメです。
...
今回の私が書く話は、WebAuthnの実装に関して、
WebAuthn APIの呼び出し部分や署名検証部分等の細かい仕様にフォーカスしてある記事はたくさんあるのですが、
じゃあこれを実際のシングルページアプリケーション（SPA）にどう組み込むんだ？という疑問がフツフツと湧いてきたので、
自分なりの考えを整理してみたのが今回の話です。

== パスワードレス認証を実装する必要があるのか

まず私のようなBtoCサービス開発者がパスワードレス実装を必要があるのか？というそもそも論から話すと、
...
さて、エモめ話はこのへんにしておいて、実装の話をしていきましょう。
...
サンプルコードは次に置いておきます。モノレポでfrontディレクトリにNuxt.jsを格納しています。
...
書こうと思いましたが、他の方がより詳しい流れを書いているので詳しく書くのをやめました。
...
== 余談1: webauthn.js プラグイン

Nuxt.jsの場合はプラグインとして切り出しておくと非常に便利です。
なおAPIの特性上SSR（ServerSideRendering）で動かないので注意してください。
